...
public class TFTensorConversionTest {
    ...
    public void testToTensor() {
        ...
        tensor =
                TFTensorConversion.toTensor(
                        new long[][] {new long[] {1, 2, 3}},
                        TensorInfo.newBuilder().setDtype(DataType.DT_INT64).build());
        assertEquals(2, tensor.shape().numDimensions());
        assertEquals(1, tensor.shape().size(0));
        assertEquals(3, tensor.shape().size(1));
        ...
        tensor =
                TFTensorConversion.toTensor(
                        new float[][] {new float[] {1, 2, 3}},
                        TensorInfo.newBuilder().setDtype(DataType.DT_FLOAT).build());
        assertEquals(2, tensor.shape().numDimensions());
        assertEquals(1, tensor.shape().size(0));
        assertEquals(3, tensor.shape().size(1));
        ...
        tensor =
                TFTensorConversion.toTensor(
                        ...
                        TensorInfo.newBuilder().setDtype(DataType.DT_DOUBLE).build());
        assertEquals(2, tensor.shape().numDimensions());
        assertEquals(1, tensor.shape().size(0));
        assertEquals(3, tensor.shape().size(1));
    }
