...
public class JavaInference implements Closeable {
    ...
    public JavaInference(MLConfig mlConfig, TableSchema inSchema, TableSchema outSchema)
            throws Exception {
        this(mlConfig.getProperties(), inSchema.getFieldNames(), outSchema.getFieldNames());
    }

    public JavaInference(
            Map<String, String> props, String[] inRowFieldNames, String[] outRowFieldNames)
            throws Exception {
        ...
        if (StringUtils.isEmpty(scheme) || scheme.equals("file")) {
            ...
            model =
                    SavedModelBundle.loader(exportDir)
            ...
            model =
                    SavedModelBundle.loader(localPath.toString())
                            .withConfigProto(configProto)
                            .withTags(TAG)
                            .load();
        } else {
        ...
        modelSig =
                MetaGraphDef.parseFrom(model.metaGraphDef().toByteArray())
        ...
        String[] inputTensorNames =
                requireConfig(props, TFConstants.TF_INFERENCE_INPUT_TENSOR_NAMES).split(SPLITTER);
        Preconditions.checkArgument(
                modelSig.getInputsMap().keySet().containsAll(Arrays.asList(inputTensorNames))
                        && Arrays.asList(inRowFieldNames)
                                .containsAll(Arrays.asList(inputTensorNames)),
        ...
        outputTensorNames =
                requireConfig(props, TFConstants.TF_INFERENCE_OUTPUT_TENSOR_NAMES).split(SPLITTER);
        Preconditions.checkArgument(
                modelSig.getOutputsMap().keySet().containsAll(Arrays.asList(outputTensorNames)),
        ...
        outputFieldNames =
                requireConfig(props, TFConstants.TF_INFERENCE_OUTPUT_ROW_FIELDS).split(SPLITTER);
        Preconditions.checkArgument(
                outputFieldNames.length == outRowFieldNames.length,
                "Output fields length mismatch");
        for (String name : outputFieldNames) {
            boolean found =
                    Arrays.asList(outputTensorNames).contains(name)
