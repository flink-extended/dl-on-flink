...
public class ExampleCoding implements Coding<Object> {
	...
	public ExampleCoding(MLContext mlContext) throws CodingException {
		...
		JSONObject jsonObject = JSONObject.parseObject(mlContext.getProperties().get(TFConstants.INPUT_TF_EXAMPLE_CONFIG));
		if (jsonObject != null) {
			this.inputConfig.fromJsonObject(jsonObject);
		}
		this.outputConfig = new ExampleCodingConfig();
		JSONObject jsonObjectOutput = JSONObject.parseObject(mlContext.getProperties().get(TFConstants.OUTPUT_TF_EXAMPLE_CONFIG));
