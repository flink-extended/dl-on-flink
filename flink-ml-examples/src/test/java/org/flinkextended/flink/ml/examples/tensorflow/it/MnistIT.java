...
public class MnistIT {
	...
	static boolean runAndVerify(MiniCluster miniCluster, MnistDist.EnvMode mode, boolean withRestart)
			throws IOException {
		...
		if (!output.contains("Program execution finished")) {
			...
			Assert.fail("MnistDist failed in mode " + mode + ", check logs in " + tmp.getAbsolutePath());
			return false;
		} else {
			File tmp = Files.createTempDir();
			miniCluster.dumpFlinkLogs(tmp);
	...
	private static String run(MiniCluster miniCluster, MnistDist.EnvMode mode, boolean withRestart) throws IOException {
		...
		String output = miniCluster.flinkRun(MnistDist.class.getCanonicalName(),
				"--zk-conn-str",
