...
/**
 * A ring buffer can be used as a queue between two threads or two processes (IPC).
 * Modified from http://psy-lob-saw.blogspot.com/2013/04/lock-free-ipc-queue.html
 *
...
public final class SpscOffHeapQueue implements Closeable {
    ...
    * */
	...
	public SpscOffHeapQueue(final int capacity) {
		this(SysUtil.UNSAFE.allocateMemory(getRequiredBufferSize(capacity) + PortableJvmInfo.CACHE_LINE_SIZE),
	...
	public static class QueueInputStream extends InputStream {
		...
		private int readOnce(byte[] b, int off, int len) throws InterruptedException {
			...
			SysUtil.UNSAFE.copyMemory(null, queue.getArrayBase() + (currentRead & queue.getMask()),
	...
	public static class QueueOutputStream extends OutputStream {
		...
		public void write(byte[] b, int off, int len) {
			...
			while (written < len) {
				...
				SysUtil.UNSAFE.copyMemory(b, Unsafe.ARRAY_BYTE_BASE_OFFSET + off + written,
