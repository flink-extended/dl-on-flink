...
public final class Docker {
    ...
    public static class ContainerBuilder {
        ...
        public boolean build() {
            ...
            return Docker.createAndStartContainer(
                    name, image, cmd, env, linkHosts, mapPorts, volumes, opts);
        }
    }
    ...
    private static boolean createAndStartContainer(
            String name,
            String image,
            String cmd,
            Map<String, String> env,
            List<String> linkHosts,
            Map<Integer, Integer> mapPorts,
    ...
    public static boolean containerExists(String name) {
        ...
        Preconditions.checkState(
                ShellExec.run(
                        "docker ps -a --format {{.Names}} --filter name=" + name,
                        stringBufConsumer(buffer)),
    ...
    public static boolean exec(String container, String cmd, StringBuffer buffer) {
        return ShellExec.run(
                String.format("docker exec %s %s", container, cmd), stringBufConsumer(buffer));
    }
    ...
    public static boolean exec(String container, String cmd, Duration timeout) {
        return ShellExec.run(
                String.format("docker exec %s %s", container, cmd), LOG::info, timeout);
    }
    ...
    public static boolean imageExist(String imageName) {
        ...
        Preconditions.checkState(
                ShellExec.run(
                        ...
                        stringBufConsumer(buffer)),
                "Failed to check existing images: " + buffer.toString());
