...
public class MiniYarnCluster {
    ...
    private static void waitHDFSReady() throws InterruptedException {
        ...
        Preconditions.checkState(
                MiniCluster.mayNeedToFixAuthorizedKeys(getYarnContainer()), "Failed to start HDFS");
    }
    ...
    private static void waitClusterReady() {
        try {
            waitHDFSReady();
            while (!ShellExec.run(
                    String.format("curl http://localhost:%d", YARN_WEBUI_HOST_PORT), true)) {
                Thread.sleep(5000);
            }
    ...
    public static MiniYarnCluster start(boolean createEnv) {
        if (!Docker.imageExist(YARN_IMAGE)) {
            ...
            Preconditions.checkState(
                    ShellExec.run(
                            String.format("cd %s && docker build -t %s .", dockerDir, YARN_IMAGE),
                            LOG::info),
    ...
    private boolean copyFromContainerToHDFS(String local, String remote) {
        return Docker.execSilently(
                getYarnContainer(), HADOOP_CMD + " fs -put -f " + local + " " + remote);
    }
    ...
    private boolean buildVirtualEnv() {
        ...
        if (tfenv.exists()) {
            ...
            String cmd =
                    String.format("sh %s", CONTAINER_WORK_HOME + "docker/flink/create_venv.sh");
    ...
    public static String getVenvHdfsPath() {
        return String.format(
                "hdfs://%s:%d%s", getYarnContainer(), HDFS_PORT, VENV_HDFS_PATH + VENV_PACK);
    }
}
