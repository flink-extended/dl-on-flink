...
/**
 * application master rpc server. management machine learning cluster life cycle. accept node
 * register, finish, failed etc.
 */
public class AppMasterServer implements Runnable {
    ...
    public AppMasterServer(MLContext mlContext) {
        rpcContactTimeout =
                Long.parseLong(
                        mlContext
                                .getProperties()
                                .getOrDefault(
                                        MLConstants.SERVER_RPC_CONTACT_TIMEOUT,
                                        MLConstants.SERVER_RPC_CONTACT_TIMEOUT_DEFAULT));
        ...
        // If we use the restart-all strategy, we should clear the old states and start all over
        // again
                        ...
                        Duration.ofMillis(
                                Long.parseLong(
                                        mlContext
                                                .getProperties()
                                                .getOrDefault(
                                                        MLConstants.HEARTBEAT_TIMEOUT,
                                                        MLConstants.HEARTBEAT_TIMEOUT_DEFAULT))));

        // am state machine
        try {
            amStateMachine =
                    AMStateMachineFactory.getAMStateMachine(
                            appMasterService, amMeta, mlContext, eventReporter);
        } catch (MLException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        }
    ...
    public void run() {
        ...
        try {
            ...
            this.server =
                    ServerBuilder.forPort(0).addService((BindableService) appMasterService).build();
            this.server.start();
            this.end.set(false);
            ...
            while (!getEnd()) {
                ...
                if (duration > rpcContactTimeout) {
                    throw new MLException(
                            String.format(
                                    "%d seconds elapsed since last grpc contact", duration / 1000));
                }
                Thread.sleep(1000);
            }
            if (error != null) {
