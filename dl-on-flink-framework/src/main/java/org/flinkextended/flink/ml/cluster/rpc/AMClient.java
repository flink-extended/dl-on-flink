...
public class AMClient extends AbstractGrpcClient {
    ...
    public SimpleResponse registerNode(long version, NodeSpec nodeSpec) {
        RegisterNodeRequest request =
                RegisterNodeRequest.newBuilder().setVersion(version).setNodeSpec(nodeSpec).build();
    ...
    public SimpleResponse heartbeat(long version, NodeSpec nodeSpec) {
        HeartBeatRequest request =
                HeartBeatRequest.newBuilder().setVersion(version).setNodeSpec(nodeSpec).build();
    ...
    public SimpleResponse nodeFinish(long version, NodeSpec nodeSpec) {
        FinishNodeRequest request =
                FinishNodeRequest.newBuilder().setVersion(version).setNodeSpec(nodeSpec).build();
    ...
    public GetClusterInfoResponse getClusterInfo(long version) {
        GetClusterInfoRequest request =
                GetClusterInfoRequest.newBuilder().setVersion(version).setMessage("").build();
    ...
    public SimpleResponse reportFailedNode(long version, NodeSpec nodeSpec) {
        return blockingStub.registerFailNode(
                RegisterFailedNodeRequest.newBuilder()
                        .setVersion(version)
                        .setNodeSpec(nodeSpec)
                        .build());
    }
     ...
     *
     * @param version machine learning job instance id
     * @param roleName machine learning cluster role name
     * @param index cluster a role index
     * @return SimpleResponse
     */
    public SimpleResponse stopJob(long version, String roleName, int index) {
        StopAllWorkerRequest request =
                StopAllWorkerRequest.newBuilder()
                        .setVersion(version)
                        .setJobName(roleName)
                        .setIndex(index)
                        .build();
    ...
    public GetFinishNodeResponse getFinishedWorker(long version) {
        GetFinishedNodeRequest request =
                GetFinishedNodeRequest.newBuilder().setVersion(version).build();
    ...
    public int getTaskIndex(long version, String scope, String key) {
        GetTaskIndexResponse response =
                blockingStub.getTaskIndex(
                        GetTaskIndexRequest.newBuilder()
                                .setScope(scope)
                                .setKey(key)
                                .setVersion(version)
                                .build());
