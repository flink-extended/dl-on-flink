...
of Deep Learning on Flink. You will run a simple Flink job locally to train a
linear model with FlinkML estimator style API.
...
using [virtualenv](https://virtualenv.pypa.io/en/latest/index.html)
...
Please refer
to [guide](https://nightlies.apache.org/flink/flink-docs-release-1.14//docs/try-flink/local_installation/)
...
In this example, we use two workers to train the model. Thus, there has to be at
least 2 slots available in the Flink cluster. To do that, you should config
the `taskmanager.numberOfTaskSlots` at `config/flink-config.yaml` to 2 with the
following command.
...
`http://localhost:8081` to view the Flink dashboard and see that the cluster is
up and running.
...
./bin/flink run \
  -py "${DL_ON_FLINK_DIR}"/examples/linear/tensorflow-estimator/flink_inference.py \
  --model-path "${MODEL_PATH}"
