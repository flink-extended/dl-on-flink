    ...
    with af.global_config_file(project_path + '/resources/workflow_config.yaml'):
        ...
        with af.config('job_2'):
            op_2 = af.user_define_operation(af.PythonObjectExecutor(
                SendEventExecutor(key='key_1', value='value_1', num=3, post_time=20)))
...
def run_workflow():
    ...
    res = af.run(project_path, dag_id='stream_trigger_batch_dag_example', scheduler_type=SchedulerType.AIFLOW)
