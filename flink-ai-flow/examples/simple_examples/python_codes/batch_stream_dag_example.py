...
class StreamPrintHelloExecutor(Executor):
    ...
    def execute(self, function_context: FunctionContext, input_list: List) -> None:
        while True:
            print("stream hello world! {}".format(self.job_name))
            time.sleep(5)
...
def build_workflow():
    with af.global_config_file(project_path + '/resources/workflow_config.yaml'):
        with af.config('job_1'):
            op_1 = af.user_define_operation(af.PythonObjectExecutor(StreamPrintHelloExecutor('job_1')))
...
def run_workflow():
    ...
    res = af.run(project_path, dag_id='batch_event_dag_example', scheduler_type=SchedulerType.AIFLOW)
