...
from airflow.models import TaskInstance, DagBag, DagRun, TaskFail
...
class TestLocalExecutor(unittest.TestCase):
    ...
    def _check_process_exist(self, pid):
        try:
    ...
    def _check_task_execution(self,
                              ti: TaskInstance,
                              session: Session = None) -> List[TaskExecution]:
        tes = session.query(TaskExecution).filter(
            TaskExecution.dag_id == ti.dag_id,
            TaskExecution.task_id == ti.task_id,
            TaskExecution.execution_date == ti.execution_date
        ).order_by(desc(TaskExecution.seq_num)).all()
        self.assertEqual(ti.pid, tes[0].pid)
        self.assertEqual(ti.state, tes[0].state)
        self.assertEqual(ti.operator, tes[0].operator)
        return tes
