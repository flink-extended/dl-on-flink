...
class KubernetesExecutor(BaseExecutor, LoggingMixin):
    ...
    def _stop_related_process(self, ti: TaskInstance) -> bool:
        command = ["pkill", "-15", "-P", ti.pid]
        kwargs = {'label_selector': f'airflow-worker={ti.external_executor_id}'}
        pod_list = self.kube_client.list_namespaced_pod(namespace=self.kube_config.kube_namespace, **kwargs)
        for pod in pod_list.items:
            if pod.metadata.name == ti.hostname:
                stream(self.kube_client.connect_get_namespaced_pod_exec,
