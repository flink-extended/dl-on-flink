...
class DagFileProcessor(AbstractDagFileProcessor, LoggingMixin):
    ...
    def _create_scheduler_job(cls, dag_id_white_list, log):
        return SchedulerJob(dag_ids=dag_id_white_list, log=log)

    @classmethod
    def _run_file_processor(cls, result_channel,
                            file_path,
                            pickle_dags,
                            dag_id_white_list,
                            thread_name,
        ...
        try:
            ...
            log.info("Started process (PID=%s) to work on %s",
                     os.getpid(), file_path)
            scheduler_job = cls._create_scheduler_job(dag_id_white_list, log=log)
   ...
   :param dag_id: if specified, only schedule tasks with this DAG ID
    ...
    def create_dag_run(self, dag, session=None):
        ...
        if dag.schedule_interval and conf.getboolean('scheduler', 'USE_JOB_SCHEDULE'):
            ...
            if dag.schedule_interval == '@deploy':
                # check file scheduled
                change_time = timezone.local_to_utc(dag.file_last_changed_on_disk)
                if last_scheduled_run is None or (last_scheduled_run < change_time):
    ...
    def process_file(self, file_path, zombies, pickle_dags=False, session=None):
        ...
        paused_dag_ids = [dag.dag_id for dag in dagbag.dags.values()
                          if dag.is_paused]
        self.log.debug("paused_dag_ids %s", paused_dag_ids)
        ...
        ti_keys_to_schedule = []
        self.log.debug("dags %s", dags)

        self._process_dags(dagbag, dags, ti_keys_to_schedule)
        self.log.debug("ti_keys_to_schedule %s", len(ti_keys_to_schedule))
