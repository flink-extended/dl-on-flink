...
import signal
import time
from airflow.models.taskstate import action_is_stop_or_restart
...
class LocalTaskJob(BaseJob):
    ...
    def _stop_task_job(self, ti):
        self.log.info("stop the task!")
        ti.task.on_kill()
        # set to
    ...
    def heartbeat_callback(self, session=None):
        ...
        if action_is_stop_or_restart(ti):
            self._stop_task_job(ti)
            return
