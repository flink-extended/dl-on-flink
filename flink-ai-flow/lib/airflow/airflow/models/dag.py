...
class DAG(BaseDag, LoggingMixin):
    ...
    def __init__(
        ...
        elif schedule_interval == '@once' or schedule_interval == '@deploy':
        ...
        self.tags = tags
        self.file_last_changed_on_disk = None
    ...
    def sync_to_db(self, owner=None, sync_time=None, session=None):
        ...
        # save task dependencies to display
        task_dependencies = {}
        for task_id, op in self.task_dict.items():
            task_dependencies[task_id] = list(op.event_dependencies())
        orm_dag.task_dependencies = json.dumps(task_dependencies)
...
class DagModel(Base):
    ...
    task_dependencies = Column(String(5000))
