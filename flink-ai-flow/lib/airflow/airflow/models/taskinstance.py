...
from airflow.exceptions import (
    AirflowException, AirflowTaskTimeout, AirflowSkipException, AirflowRescheduleException, StopTaskException
)
...
class TaskInstance(Base, LoggingMixin):
    ...
    @property
    def task_execution_url(self):
        iso = self.execution_date.isoformat()
        base_url = conf.get('webserver', 'BASE_URL')
        relative_url = '/taskexecution?execution_date={iso}&task_id={task_id}&dag_id={dag_id}'.format(
    ...
    def _run_raw_task(
        ...
        task_start_time = timezone.utcnow()
        ...
        try:
            if not mark_success:
                ...
                def signal_handler(signum, frame):
                    ...
                    task_copy.on_kill()
                    # todo check kill if by stop command
                    raise StopTaskException("Task received SIGTERM signal")
            ...
            self.log.info("dag_id=%s, task_id=%s, execution_date=%s stopped!",
        ...
        session.commit()
        # add task execution
        self._add_task_execution(task_start_time, session)

    def _add_task_execution(self, start_time, session=None):
        try:
            copy_ti = session.query(TaskInstance).filter(
                TaskInstance.dag_id == self.dag_id,
                TaskInstance.task_id == self.task_id,
                TaskInstance.execution_date == self.execution_date).first()
            copy_ti.start_date = start_time
            copy_ti.end_date = timezone.utcnow()
            copy_ti.duration = (copy_ti.end_date - copy_ti.start_date).total_seconds()
            te = TaskExecution(copy_ti)
            te.add_to_db(session=session)
            self.log.debug("add task execution {0} to db".format(te))
