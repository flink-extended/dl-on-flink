...
class DefaultEventMetHandler(EventMetHandler):
    ...
    def handle_event(self, event: Event, ti: TaskInstance, ts: TaskState, session=None):
        return TaskAction.START


class EventOperator(LoggingMixin):
    """
    EventOperator introduces a new attribute _events to hold user-defined events.
    """

    def __init__(self, ):
        """
        Attribute _signals represents events defined on the operator.
        """
        self._events: Set = set()
        self._events_met_handler: EventMetHandler = None

    def add_event_dependency(self, event_key, event_type):
        """
        Set the user-defined event.
        :param: event: Specific user-defined event.
        """
        self._events.add((event_key, event_type))

    def event_dependencies(self) -> Set:
        """
        Return the set of events for the operator.

        :return: Set of events defined on the operator.
        """
        return self._events

    def remove_event_dependency(self, event_key, event_type):
        """
        remove the event
        :param: event: Specific user-defined event.
        """
        self._events.discard((event_key, event_type))

    def set_event_met_handler(self, handler: EventMetHandler):
        """
        :param handler: EventMetHandler
        :return: TaskAction
        """
        self._events_met_handler = handler

    def event_met_handler(self):
        """
        :return: handler type: EventMetHandler
        """
        return self._events_met_handler


@functools.total_ordering
class BaseOperator(EventOperator):
    """
