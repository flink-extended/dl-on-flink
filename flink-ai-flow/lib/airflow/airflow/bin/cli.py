...
@cli_utils.action_logging
def event_scheduler(args):
    py2_deprecation_waring()
    print(settings.HEADER)
    job = jobs.EventSchedulerJob(
        dag_id=args.dag_id,
        subdir=process_subdir(args.subdir),
        run_duration=args.run_duration,
        num_runs=args.num_runs,
    ...
    if args.daemon:
        pid, stdout, stderr, log_file = setup_locations("event_scheduler",
                                                        args.pid,
                                                        args.stdout,
                                                        args.stderr,
                                                        args.log_file)
    ...
    subparsers = (
        {
            ...
            'func': event_scheduler,
            'help': "Start a event_scheduler instance",
            'args': ('dag_id_opt', 'subdir', 'run_duration', 'num_runs',
                     'do_pickle', 'pid', 'daemon', 'stdout', 'stderr',
                     'log_file'),
        }, {
