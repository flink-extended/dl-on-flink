...
<template>
  <div class="flowchart flowchart-wrapper" @mousemove="mousemove">
    <div class="flowchart-links">
      <!-- 节点图路径path -->
      <svg ref="svg">
        <path v-for="(link, index) in chartLinks" :key="index"
    ...
    <ul class="flowchart-ops">
      <li class="flowchart-operator"
        ...
        <div class="flowchart-operator-inputs-outputs">
          <div class="flowchart-operator-inputs" v-if="op.properties.inputs">
            <div class="flowchart-operator-connector">
              <div class="flowchart-operator-connector-arrow"
                   :ref="'ins' + index" @click.stop="clickIns(op, $event)"
          ...
          <div class="flowchart-operator-outputs" v-if="op.properties.outputs">
            <div class="flowchart-operator-connector">
              <div class="flowchart-operator-connector-arrow"
                   :ref="'outs' + index" @click.stop="clickOuts(op, $event)"
                   :class="{noedit: !canUserEditLinks}">
              </div>
            </div>
          </div>
        </div>
        <div v-if="!op.properties.sourceFlag"
        ...
        <div v-else-if="op.properties.sourceFlag"
...
export default {
    ...
    chartLinks () {
      ...
      this._operators.forEach((o, index) => {
        o.parents.forEach((p, i) => {
          ...
          let parent = this._operators.find(op => op.id === id)
          let isSignal = ((o.signals || []).find(v => v.id === id) || {}).isSignal === 1
          if (parent && parent.outs && o.ins) {
            ...
            let link = this.$refs.svg && this.$refs.svg.querySelector('link' + linkNum)
            link && link.remove()
          }
        })
      })
  ...
  methods: {
    ...
    dragOperator (x, y, id) {
      let operator = this._operators.find(o => o.id === id)
      let chartOperator = this.chartOperators[id]
    ...
    clickOuts (from, ev) {
      if (!this.drawing && this.canUserEditLinks) {
        this.drawing = from
        let domOperator = (this.$refs['op' + from.id] && this.$refs['op' + from.id][0]) || {
          offsetWidth: 84,
          offsetHeight: 50
        }
    ...
    drawLink (id, from, to) {
      ...
      if (from && to) {
        this.$nextTick(_ => {
          let opo = this.$refs['op' + from.id]
          opo = opo && opo[0]
          let outs = this.$refs['outs' + from.id]
          outs = outs && outs[0]
          let opi = this.$refs['op' + to.id]
          opi = opi && opi[0]
          let ins = this.$refs['ins' + to.id]
          ins = ins && ins[0]
          if (opo && outs && opi && ins) {
            let inset = 2
            let fromX, fromY, toX1, toY1, toX2, toY2, toX3, toY3
            ...
            let bezierIntensity = Math.min(this.linkBending, Math.max(Math.abs(fromY - toY2), Math.abs(fromX - toX2)))
            let svg = this.$refs.svg
            let path = svg.querySelector('.link' + id)
            // 闭环节点
            if (close) {
    ...
    delLink (linkId = this.selectedLinkId) {
      let linkData = this.chartLinks[linkId]
      if (linkData) {
        let parent = this.chartOperators[linkData.fromOperator].properties
        let child = this.chartOperators[linkData.toOperator].properties
