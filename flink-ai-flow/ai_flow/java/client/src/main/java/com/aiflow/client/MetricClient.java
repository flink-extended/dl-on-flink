...
import com.aiflow.proto.Message.MetricMetaProto;
import com.aiflow.proto.MetricServiceGrpc;
import com.aiflow.proto.MetricServiceOuterClass;
import com.aiflow.proto.MetricServiceOuterClass.MetricSummaryRequest;
import com.aiflow.proto.MetricServiceOuterClass.MetricMetaRequest;
...
import static com.aiflow.util.Transform.int64Value;
...
public class MetricClient {
    ...
    public MetricMeta registerMetricMeta(String name, MetricType metricType, String projectName, String description, String datasetName, String modelName,
        ...
        MetricMetaProto metricMetaProto = MetricMetaProto.newBuilder().setMetricName(stringValue(name)).setMetricType(metricType.getMetricType())
                .setProjectName(stringValue(projectName)).setMetricDesc(stringValue(description)).setDatasetName(stringValue(datasetName))
                .setModelName(stringValue(modelName)).setJobName(stringValue(jobName)).setStartTime(int64Value(startTime))
                .setEndTime(int64Value(endTime)).setUri(stringValue(uri)).setTags(stringValue(tags)).putAllProperties(properties).build();
        MetricMetaRequest request = MetricMetaRequest.newBuilder().setMetricMeta(metricMetaProto).build();
        MetricServiceOuterClass.MetricMetaResponse response = metricServiceStub.registerMetricMeta(request);

        if (0 == response.getReturnCode()) {
             return MetricMeta.buildMetricMeta(response.getMetricMeta());
        } else {
            throw new AIFlowException(String.valueOf(response.getReturnCode()), response.getReturnMsg());
        }
    }
    ...
    public MetricMeta updateMetricMeta(String name, String projectName, String description, String datasetName, String modelName,
                                       String jobName, long startTime, long endTime, String uri, String tags, Map<String, String> properties) throws AIFlowException {
        MetricMetaProto metricMetaProto = MetricMetaProto.newBuilder().setMetricName(stringValue(name))
                .setProjectName(stringValue(projectName)).setMetricDesc(stringValue(description)).setDatasetName(stringValue(datasetName))
                .setModelName(stringValue(modelName)).setJobName(stringValue(jobName)).setStartTime(int64Value(startTime))
                .setEndTime(int64Value(endTime)).setUri(stringValue(uri)).setTags(stringValue(tags)).putAllProperties(properties).build();
        MetricMetaRequest request = MetricMetaRequest.newBuilder().setMetricMeta(metricMetaProto).build();
        MetricServiceOuterClass.MetricMetaResponse response = metricServiceStub.updateMetricMeta(request);
    ...
    public boolean deleteMetricMeta(String metricName) {
        MetricServiceOuterClass.MetricNameRequest request = MetricServiceOuterClass.MetricNameRequest.newBuilder().setMetricName(metricName).build();
    ...
    public MetricMeta getMetricMeta(String metricName) throws AIFlowException {
        MetricServiceOuterClass.MetricNameRequest request = MetricServiceOuterClass.MetricNameRequest.newBuilder().setMetricName(metricName).build();
        MetricServiceOuterClass.MetricMetaResponse response = metricServiceStub.getMetricMeta(request);
        if (0 == response.getReturnCode()) {
            return MetricMeta.buildMetricMeta(response.getMetricMeta());
        } else {
            throw new AIFlowException(String.valueOf(response.getReturnCode()), response.getReturnMsg());
    ...
    public List<MetricMeta> listDatasetMetricMetas(String datasetName, String projectName) throws AIFlowException {
        MetricServiceOuterClass.ListDatasetMetricMetasRequest request = MetricServiceOuterClass.ListDatasetMetricMetasRequest.newBuilder()
                .setDatasetName(datasetName).setProjectName(stringValue(projectName)).build();
        MetricServiceOuterClass.ListMetricMetasResponse response = metricServiceStub.listDatasetMetricMetas(request);
        if (0 == response.getReturnCode()) {
    ...
    public List<MetricMeta> listModelMetricMetas(String modelName, String projectName) throws AIFlowException {
        MetricServiceOuterClass.ListModelMetricMetasRequest request = MetricServiceOuterClass.ListModelMetricMetasRequest.newBuilder()
                .setModelName(modelName).setProjectName(stringValue(projectName)).build();
        MetricServiceOuterClass.ListMetricMetasResponse response = metricServiceStub.listModelMetricMetas(request);
        if (0 == response.getReturnCode()) {
            return MetricMeta.buildMetricMetas(response.getMetricMetasList());
        } else {
            throw new AIFlowException(String.valueOf(response.getReturnCode()), response.getReturnMsg());
    ...
    public MetricSummary registerMetricSummary(String metricName, String metricKey, String metricValue, long metricTimestamp,
        ...
        Message.MetricSummaryProto metricSummaryProto = Message.MetricSummaryProto.newBuilder().setMetricName(stringValue(metricName))
                .setMetricKey(stringValue(metricKey)).setMetricValue(stringValue(metricValue)).setMetricTimestamp(int64Value(metricTimestamp))
                .setModelVersion(stringValue(modelVersion)).setJobExecutionId(stringValue(jobExecutionId)).build();
        MetricSummaryRequest request = MetricSummaryRequest.newBuilder().setMetricSummary(metricSummaryProto).build();
        MetricServiceOuterClass.MetricSummaryResponse response = metricServiceStub.registerMetricSummary(request);
    ...
    public MetricSummary updateMetricSummary(long uuid, String metricName, String metricKey, String metricValue, long metricTimestamp,
                                             String modelVersion, String jobExecutionId) throws AIFlowException {
        Message.MetricSummaryProto metricSummaryProto = Message.MetricSummaryProto.newBuilder().setUuid(uuid).setMetricName(stringValue(metricName))
                .setMetricKey(stringValue(metricKey)).setMetricValue(stringValue(metricValue)).setMetricTimestamp(int64Value(metricTimestamp))
                .setModelVersion(stringValue(modelVersion)).setJobExecutionId(stringValue(jobExecutionId)).build();
        MetricSummaryRequest request = MetricSummaryRequest.newBuilder().setMetricSummary(metricSummaryProto).build();
        MetricServiceOuterClass.MetricSummaryResponse response = metricServiceStub.updateMetricSummary(request);
    ...
    public boolean deleteMetricSummary(long uuid) {
        MetricServiceOuterClass.UuidRequest request = MetricServiceOuterClass.UuidRequest.newBuilder().setUuid(uuid).build();
    ...
    public MetricSummary getMetricSummary(long uuid) throws AIFlowException {
        MetricServiceOuterClass.UuidRequest request = MetricServiceOuterClass.UuidRequest.newBuilder().setUuid(uuid).build();
        MetricServiceOuterClass.MetricSummaryResponse response = metricServiceStub.getMetricSummary(request);
        if (0 == response.getReturnCode()) {
            return MetricSummary.buildMetricSummary(response.getMetricSummary());
        } else {
            throw new AIFlowException(String.valueOf(response.getReturnCode()), response.getReturnMsg());
    ...
    public List<MetricSummary> listMetricSummaries(String metricName, String metricKey, String modelVersion,
        ...
        MetricServiceOuterClass.ListMetricSummariesRequest.Builder builder = MetricServiceOuterClass.ListMetricSummariesRequest.newBuilder();
        if (metricName != null) {
        ...
        MetricServiceOuterClass.ListMetricSummariesRequest request = builder.setStartTime(int64Value(startTime))
                .setEndTime(int64Value(endTime)).build();
        MetricServiceOuterClass.ListMetricSummariesResponse response = metricServiceStub.listMetricSummaries(request);
        if (0 == response.getReturnCode()) {
